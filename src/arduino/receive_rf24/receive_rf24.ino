 ...
 *   - Uses a 5-byte pipe address to uniquely identify the communication
 *     channel.
 *   - Customizable RF settings such as power level, data rate, and
 *     communication channel.
 *   - Continuously listens for incoming payload data (a float) and displays it
 * via Serial Monitor.
 ...
 *       IDE or external tools like PuTTY) Hardware:
...
// functions
void callback(); // interrupt callback method
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(IRQ_PIN), callback, FALLING);
